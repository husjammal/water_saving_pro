...
final data = csvList.skip(1).map((row) {
if (row.length >= 4) {
try {
...
return null;
}).where((row) => row != null).cast<List<dynamic>>().toList();
_csvData = data;
_logger.i('Loaded ${_csvData.length} records from CSV');
} else {
_logger.i('No CSV file found');
}
} catch (e) {
...
_lastTimestamp = _csvData.map((row) => row[0] as int).reduce((a, b) => a > b ? a : b);
